...
if __name__ == '__main__':
    ...
    for n, data in enumerate(tqdm(test_set_selected)):
        ...
        pocket_sdf_dir = os.path.join(sdf_dir, gen_file_name)
        if save_sdf_flag and not os.path.exists(pocket_sdf_dir):
            os.makedirs(pocket_sdf_dir)
        ...
        with torch.no_grad():
            ...
            while num_samples > 0 and try_num > 0:
                ...
                current_batch_size = min(num_samples, batch_size)

                if try_num < 10:
                    num_points_fix = None
                num_points_fix = None  # only for no spatial
                data_list, _ = construct_dataset_pocket(current_batch_size, batch_size, dataset_info,
                        ...
                        try:
                            ...
                            if g_smile is not None:
                                ...
                                if len(g_smile) < 4:
                                    raise MolReconsError()
                                if save_sdf_flag:
                                    unique_gen_file_name = f'mol_{sample_idx}.sdf'
                                    save_sdf(gmol, pocket_sdf_dir, unique_gen_file_name)
                                    sample_idx += 1
                                valid += 1
                                num_samples -= 1
