...
def train(it):
        ...
        logger.info('[Train] Epoch %05d | Loss %.2f | Loss(Global) %.2f | Loss(Local) %.2f | Loss(node_global) %.2f | Loss(node_local) %.2f | Loss(vae_KL) %.2f |Grad %.2f | LR %.6f' % (
                it, avg_loss, avg_loss_global, avg_loss_local, avg_loss_node_global, avg_loss_node_local, loss_vae_KL, orig_grad_norm, optimizer.param_groups[0]['lr'],
            ))
        ...
        writer.add_scalar('train/lr_local', optimizer.param_groups[0]['lr'], it)
...
if __name__ == '__main__':
    ...
    # Optimizer
    # Removed unused optimizer_global, scheduler_global, optimizer_local, scheduler_local definitions
