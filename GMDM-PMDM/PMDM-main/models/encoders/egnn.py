...
class EGNN_Sparse_Network(nn.Module):
    ...
    def forward(self, z, pos, edge_index, edge_attr, batch, ligand_batch, context, linker_mask=None,
        ...
        # # no CA
        # h_ligand = self.ligandemb(z[:len(ligand_batch),:])+context
        # h_protein = self.proteinemb(z[len(ligand_batch):,:])
        ...
        # # CA inside
        # h_ligand = self.ligandemb(z[:len(ligand_batch),:])+context[:len(ligand_batch),:]
        # h_protein = self.proteinemb(z[len(ligand_batch):,:])
        # # h_protein = self.proteinemb(context[len(ligand_batch):,:])
        # h_ligand  = self.atten_layer(h_ligand, context[len(ligand_batch):,:])
        # h = torch.cat([h_ligand,h_protein],dim=0) 
