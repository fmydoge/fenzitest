...
def xyz_to_dat(pos, edge_index, num_nodes, use_torsion=False):
    ...
    Args:
        ...
        use_torsion: If set to :obj:`True`, will return distance, angle and torsion, otherwise only return distance and
        angle (also retrun some useful index). (default: :obj:`False`)
    """
    j, i = edge_index  # j->i
    ...
    def __init__(self, num_spherical, num_radial, cutoff=5.0,
        ...
        for i in range(self.num_spherical):
            if i == 0:
                ...
                self.sph_funcs.append(lambda x, y: torch.zeros_like(x) + torch.zeros_like(y) + sph1(0, 0))
...
class init(torch.nn.Module):
    def __init__(self, num_radial, hidden_channels, act=swish, use_node_features=True):
        ...
        if self.use_node_features:
            ...
            self.emb = nn.Linear(17, hidden_channels)  # 16 or 8
...
class update_v(torch.nn.Module):
    ...
    #     v = self.lin(v)
    #     return v
    def forward(self, e, i, batch):
        _, e2 = e  # e2 是边的特征
        batch = batch.to(e2.device)
        num_atoms = batch.shape[0]  # 计算原子数
        i = i - batch.min()
        # 确保所有原子都参与
        v = torch.ones((num_atoms, e2.shape[1]), device=e2.device)  # 初始化 (num_atoms, feature_dim)
        v = scatter(e2, i, dim=0, out=v)  # 仅更新有边的原子
        v = self.lin_up(v)
        for lin in self.lins:
            v = self.act(lin(v))
        v = self.lin(v)
...
class SphereNet(torch.nn.Module):
    r"""
        ...
        Args:
            energy_and_force (bool, optional): If set to :obj:`True`, will predict energy and take the negative of the
            derivative of the energy with respect to the atomic positions as predicted forces. (default: :obj:`False`)
            ...
            basis_emb_size_dist (int, optional): Embedding size used in the basis transformation of distance.
            (default: :obj:`8`)
            basis_emb_size_angle (int, optional): Embedding size used in the basis transformation of angle.
            (default: :obj:`8`)
            basis_emb_size_torsion (int, optional): Embedding size used in the basis transformation of torsion.
            (default: :obj:`8`)
            ...
            num_before_skip (int, optional): Number of residual layers in the interaction blocks
            before the skip connection. (default: :obj:`1`)
            num_after_skip (int, optional): Number of residual layers in the interaction blocks
            before the skip connection. (default: :obj:`2`)
            num_output_layers (int, optional): Number of linear layers for the output blocks. (default: :obj:`3`)
            act: (function, optional): The activation funtion. (default: :obj:`swish`)
            output_init: (str, optional): The initialization fot the output.
            It could be :obj:`GlorotOrthogonal` and :obj:`zeros`. (default: :obj:`GlorotOrthogonal`)

    """

    def __init__(
            self, energy_and_force=False, cutoff=10.0, num_layers=4,
