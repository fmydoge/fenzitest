...
from utils.utils import prepare_context, compute_mean_mad
...
def train(it):
    ...
    with tqdm(total=len(train_loader), desc='Training') as pbar:
        for batch in train_loader:
            ...
            pbar.set_postfix({'loss': f'{loss_val.item():.2f}'})
            pbar.update(1)

            torch.cuda.empty_cache()  # 每个batch后清一下缓存
    ...
    if dist.get_rank() == 0:
        logger.info(
            f'[Train] Iter {it:05d} | Loss {loss_val.item():.2f} | '
...
if __name__ == '__main__':
    ...
    if resume:
        ...
        symlink_path = os.path.join(log_dir, os.path.basename(resume_from.rstrip("/")))

        # **如果目标已经存在，先删除它**
        if os.path.exists(symlink_path) or os.path.islink(symlink_path):
            os.remove(symlink_path)

        os.symlink(os.path.realpath(resume_from), symlink_path)
    else:
    ...
    for it in range(start_iter, config.train.max_iters + 1):
        #train_sampler.set_epoch(it)  # 让 `DistributedSampler` 每轮 shuffle
        start_time = time.time()
        train(it)
        torch.cuda.empty_cache() #每轮后释放显存
