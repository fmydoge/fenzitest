...
from ..encoder import SchNetEncoder, get_edge_encoder, EGNNSparseNetwork, SphereNet
from ..geometry import get_distance, eq_transform, get_angle_index, get_dihedral_index, get_angle, get_dihedral
...
class MDMFullDP(nn.Module):
    ...
    def net(self, atom_type, pos, bond_index, bond_type, batch, time_step,
        ...
        # # 计算 angle_index
        # angle_index = get_angle_index(edge_index, local_edge_mask)
        # # 计算 dihedral_index
        # dihedral_index = get_dihedral_index(angle_index, edge_index, local_edge_mask)
        # # 计算 angle_angle (键角值)
        # angle_angle = get_angle(pos, angle_index)
        # # 计算 dihedral_angle (二面角值)
        # dihedral_angle = get_dihedral(pos, dihedral_index)
        ...
        # if torch.isnan(edge_attr_global).any():
        #     print("⚠️ Warning: edge_attr_global contains NaN values!")
        #     print(edge_attr_global)
        ...
        # if torch.isnan(node_attr_global).any():
        #     print("⚠️ Warning: node_attr_global contains NaN values!")
        """
        ...
        # SphereNet
        _,node_attr_local, _ = self.encoder_local(
            z=atom_type,
            pos=pos,
            edge_index=edge_index[:, local_edge_mask],
            batch=batch
    ...
    def forward(self, batch, context=None, return_unreduced_loss=False, return_unreduced_edge_loss=False,
        ...
        pos = torch.nan_to_num(pos, nan=0.0, posinf=1.0, neginf=-1.0)  # 预防 NaN 传播
        ...
        a_edge = torch.clamp(a_edge, min=1e-8)  # 避免 sqrt(0) 产生 NaN
        ...
        # if torch.isnan(loss_global).any():
        #     print("Warning: loss_global is NaN")
        #     print(f"node_eq_global: {node_eq_global}")
        #     print(f"target_pos_global: {target_pos_global}")
