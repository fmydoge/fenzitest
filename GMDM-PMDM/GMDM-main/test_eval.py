                    ...
                    for m in range(batch_size):
                        ...
                        if smile is not None:
                            ...
                            if sdf:
                                ...
                                writer.close()
                            # if sdf and smile is not None:
                            #     try:
                            #         mol_from_smile = Chem.MolFromSmiles(smile)
                            #         mol_h = Chem.AddHs(mol_from_smile)  # 添加显式氢原子
                            ...
                            #             writer.close()
                            #         else:
                            #             logger.warning(f"3D构象生成失败 for smile: {smile}")

                            #     except Exception as e:
                            #         logger.warning(f"SMILES转3D分子失败: {smile}, 错误信息: {e}")
